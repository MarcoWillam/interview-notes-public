...
import { localStore, type Preference } from '@/lib/local/store';
...
export function PreferenceLibrary({
  open,
  onClose,
  library,
  current,
  ...
  onClose: () => void;
  library: Library;
  current: Omit<Preference, 'id' | 'name'>;
  onApply: (p: Preference) => void;
  onError: (message: string) => void;
}) {
  ...
  async function act(fn: () => Promise<void>) {
    ...
    try {
      ...
      const message = e instanceof Error ? e.message : '模板操作失败';
      setError(message);
      onError(message);
    } finally {
      running.current = false;
  ...
  return (
    <>
      ...
      >
        <DialogContent className="library-dialog" showCloseButton={false}>
          <div className="dialog-heading">
            <DialogTitle>面试偏好模板</DialogTitle>
            <button
              className="icon-button"
              aria-label="关闭偏好模板"
              disabled={pending}
          ...
          <DialogDescription>
            复用岗位要求、评估维度和重点考察事项。模板仅保存在当前浏览器，不包含候选人和简历。
          </DialogDescription>
          <label htmlFor="preference-name">把当前设置存为模板</label>
          {error && (
            <p className="message error" role="alert">
              {error}
            </p>
          ...
          <input
            ...
            placeholder="例如：高级产品经理 · 一面"
          />
          <button
            className="primary-button"
            disabled={pending || !name.trim() || !current.role.trim()}
            onClick={() =>
              void act(async () => {
                if (!current.requirements.trim())
                  throw new Error('请先填写岗位要求');
                const id =
                  library.preferences.find((p) => p.name === name.trim())?.id ||
                  crypto.randomUUID();
                await library.savePreference({
                  ...current,
                  id,
                  name: name.trim(),
                });
          ...
          <p className="small-note">
            同名模板将更新；应用模板会清除旧 AI 评估并重置结论确认状态。
          </p>
          <div className="local-list">
            {library.preferences.map((p) => (
              <article key={p.id} className="local-row">
                <div>
                  <strong>{p.name}</strong>
                  <p>
                    {p.role} · {p.dimensionText}
                  </p>
                </div>
                <div className="button-row">
                  <button
      ...
      >
        <AlertDialogContent>
          <AlertDialogTitle>删除这个偏好模板？</AlertDialogTitle>
          <AlertDialogDescription>
            已应用到面试中的要求不受影响。
          </AlertDialogDescription>
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              onClick={() => {
                const id = deleting!;
                setDeleting(null);
                void act(() => library.removePreference(id));
              }}
            >
              删除模板
            </AlertDialogAction>
