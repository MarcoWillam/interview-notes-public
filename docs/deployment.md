...
owner 的本机连接器已在 `<USER_HOME>/Downloads/interview-connector` 原位升级，保留 `.local/connector.json` 与 `works/`，无需新配对码；旧程序保存在同级 `interview-connector.backup-20260914-1`。升级后连接器以后台进程运行，服务器确认版本 `2026.9.14-3`、协议 5、在线状态正常。其他面试官需要从网页下载并替换一次程序文件，同时保留各自的 `.local` 与 `works`；完成这次协议 5 升级后，普通提纲规则更新不再要求替换连接器。
