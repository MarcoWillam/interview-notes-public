...
const previousBuiltInRoleTemplatesV6 = builtInRoleTemplates
  .slice(0, 2)
  .map((template, index) => ({
    ...template,
    reportRequirements:
...
export function replacementForPreviousBuiltInRoleTemplate(value: unknown) {
  const previous = previousBuiltInRoleTemplatesV6.find(
    ({ id }) =>
      typeof value === 'object' &&
      value !== null &&
      (value as { id?: unknown }).id === id,
  );
  if (!previous || !exactlyMatchesRoleTemplate(value, previous))
    return undefined;
  return builtInRoleTemplates.find(({ id }) => id === previous.id);
}
