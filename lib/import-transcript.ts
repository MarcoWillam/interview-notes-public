/** Read a bounded UTF-8 Markdown or TXT file as literal text; never render HTML. */
export async function importTranscript(
  file: Pick<File, 'name' | 'size' | 'arrayBuffer'>,
): Promise<string> {
  if (!/\.(?:md|txt)$/i.test(file.name))
    throw new Error('面试记录仅支持 .md、.txt 文件。');
  ...
  try {
    ...
    throw new Error('请将面试记录保存为 UTF-8 编码的 .md 或 .txt 文件后重试。');
  }
  text = text.replace(/\r\n?/g, '\n');
  ...
  if (/[\u0000-\u0008\u000b\u000c\u000e-\u001f\u007f]/u.test(text))
    throw new Error('文件包含非文本内容，请上传纯文本文件。');
